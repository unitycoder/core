...
namespace VoxelGraph.Processor
{
	public class VoxelGraphProcessor : BaseGraphProcessor
	{
		List<BaseNode> processList;
		List<StartNode> startNodeList;
		List<OutputNode> outputNodeList;
		...
		public VoxelGraphProcessor(BaseGraph graph) : base(graph)
		{
		}

		public OutputNode OutputNode => outputNodeList.FirstOrDefault();

		public override void UpdateComputeOrder()
		{
			// Gather start nodes:
			startNodeList = graph.nodes.Where(n => n is StartNode).Select(n => n as StartNode).ToList();
			outputNodeList = graph.nodes.Where(n => n is OutputNode).Select(n => n as OutputNode).ToList();
		...
		public override void Run()
		{
			...
			if (startNodeList.Count != 1 || outputNodeList.Count != 1)
			{
				Debug.LogError("There must be 1 start node AND 1 output node!");
				return;
			}
			else
			{
