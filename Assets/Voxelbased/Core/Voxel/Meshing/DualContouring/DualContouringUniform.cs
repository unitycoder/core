//-----------------------------------------------------------------------
// <copyright file="DualContouringUniform.cs" company="theSoenke">
...
namespace VoxelbasedCom
{
    ...
    public class DualContouringUniform : MeshBuilder
    {
        ...
        private void CalculatePoints(Row row)
        {
            for (int x = 0; x < Row.size; x++)
            {
                for (int z = 0; z < Row.size; z++)
                {
                    ...
                    Vector3 pos = new Vector3((row.pos.x + x - 1) * voxelSize, row.pos.y, (row.pos.z + z - 1) * voxelSize);
                    pos += offset;
                    float densityPoint = GetDensity(pos.x,pos.y,pos.z);
        ...
        private void GenerateVertex(Vertex vertex, Voxel[] corners)
        {
            ...
            for (int i = 0; i < 12; i++)
            {
                ...
                else
                {
                    ...
                    {
                        IntersectXAxis(corners[n1], corners[n2], ref p);
                        
                    }
                    if (Mathf.Abs(vDiff.y) > ToleranceDensity)
                    {
                        IntersectYAxis(corners[n1], corners[n2], ref p);
                        
                    }
                    if (Mathf.Abs(vDiff.z) > ToleranceDensity)
                    {
                        IntersectZAxis(corners[n1], corners[n2], ref p);
                        
                    }
                    data.intersectionPoints.Add(p);
                }
        ...
        private void IntersectXAxis(Voxel p0, Voxel p1, ref Vector3 pOut)
        {
            float xa, xb;
        ...
        /// </summary>
        /// <returns>Interpolated vector</returns>
        private void IntersectYAxis(Voxel p0, Voxel p1, ref Vector3 pOut)
        {
            float ya, yb;
        ...
        /// </summary>
        /// <returns>Interpolated vector</returns>
        private void IntersectZAxis(Voxel p0, Voxel p1, ref Vector3 pOut)
        {
            float za, zb;
